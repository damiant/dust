  ...
  for (let item of json) {
    ...
    if (item.print_description) {
      ...
      if (item.print_description[0].toUpperCase() != item.print_description[0]) {
        item.print_description = item.print_description.charAt(0).toUpperCase() + item.print_description.slice(1);
      }
    }
    if (item.uid == '44098') {
...
async function processYears(localWrite: boolean, years: string[]) {
  ...
  for (const year of years) {
    ...
    const campsChanged = await download('camp', year, 'camps', `ttitd-${year}`, {
      fixName: true,
      fixLocation: true,
      removeEmail: true,
      localWrite,
    });
    const eventsChanged = await download('event', year, 'events', `ttitd-${year}`, {
      fixOccurrence: true,
      fixTitle: true,
      fixUid: true,
      localWrite,
