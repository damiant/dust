...
export class CacheService {
    ...
    public async download(status: WritableSignal<CacheStatus>, abort: WritableSignal<boolean>): Promise<string> {
        ...
        if (ok) {
            ...
            const revision = await this.api.getRevision(this.settings.settings.datasetId);
            await this.settings.setInteger(`${this.settings.settings.datasetId}-cachedRevision`, revision ? revision.revision : 0);
        }
    ...
    private async cacheFiles(urls: string[], status: WritableSignal<CacheStatus>, type: CacheType, abort: WritableSignal<boolean>): Promise<boolean> {
        let id = 1;
